...
-- Кстати говоря, в приведенном примере используется закрытый канал, так что сообщение STOP должно отправляться с той же учетной записи пользователя, которая выполняет бесконечный цикл.
-- Также следует заметить, что пространство имен базы данных для закрытых каналов глобально поотношению ко всем сеансам текущего пользователя.
-- Следовательно, если вы захотите, чтобы в бесконечном цикле выполнялось сразу несколько программ, необходимо реализовать дополнительную логику для
--    (1) создания имен каналов, уникальных для каждого сеанса
--    (2) определения имен каналов для отправки команды STOP
------------------------------------------------------------------
DECLARE
